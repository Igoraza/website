...
function renderJobs(jobs) {
    ...
    const expiredJobsContainer = document.getElementById('expired-jobs-container');
    
    // Clear the containers before rendering
    jobsContainer.innerHTML = '';
    expiredJobsContainer.innerHTML = '';
    
    jobs.forEach(job => {
        if (job.is_expired) {
            expiredJobsContainer.innerHTML += createJobHTML(job);
        } else {
            jobsContainer.innerHTML += createJobHTML(job);
        }
    });
}
